...
class TruckAgent(PubSubMixin, Agent):
    ...
    async def log_delivery(self, delivered_quantity):
        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        log_data = [current_time, 'Entrega', delivered_quantity]
    ...
    class TruckDeliverBehav(PeriodicBehaviour):
        ...
        async def run(self):
            ...
            try:
                await self.agent.pubsub.publish(PUBSUB_JID, STOCK_PUBSUB, str(deliver_amount))
            except Exception:
                print(f"[{self.agent.name}] {STOCK_PUBSUB} PubSub Error")

            print(f"[{self.agent.name}] Delivered {deliver_amount}")

            await self.agent.log_delivery(deliver_amount)
    ...
    class ProcessStockBehav(CyclicBehaviour):
        ...
        async def run(self):
            ...
            if self.agent.stock > 0:
                ...
                agenteTrabalhando = self.agent.name
...
with open('fryer_data.csv', mode='w', newline='') as csv_file:
    fieldnames = ['Data', 'Quantidade Produzida', 'Quantidade em estoque']
    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
    writer.writeheader()

    while True:
        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        quantidade_produzida = random.randint(0, 100)
        writer.writerow({'Data': current_time, 'Quantidade Produzida': quantidade_produzida, 'Quantidade em estoque': quantidade_estoque})
        time.sleep(5)